...
list() {
	file_list=$(curl -ks -H ${ARTIFACTORY_TOKEN} "${DOWNLOAD_URL}")
...
isLatestUpdate() {
	...
	echo $JCK_WITHOUT_BACKSLASH
	GIT_URL="https://raw.github.ibm.com/runtimes/JCK$JCK_VERSION-unzipped/main/update_ibm.txt"

	curl -o "update_ibm.txt" $GIT_URL -H "Authorization: token $GIT_TOKEN"
	echo -e "JCK version in update_ibm.txt:\n$(cat update_ibm.txt)\n"
	
	if grep -q "$JCK_WITHOUT_BACKSLASH" update_ibm.txt; then
		...
		echo "update update_ibm.txt to $JCK_WITHOUT_BACKSLASH"
		echo "- $JCK_WITHOUT_BACKSLASH" > update_ibm.txt
		getJCKSources
	fi
}
...
getJCKSources() {
	...
	if [ "${DOWNLOAD_URL}" != "" ]; then
		for file in "${file_names_array[@]:1}"; do
			...
			executeCmdWithRetry "${file##*/}" "_ENCODE_FILE_NEW=UNTAGGED curl -OLJSk -H ${ARTIFACTORY_TOKEN} $url"
